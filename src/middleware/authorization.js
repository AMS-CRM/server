...
module.exports = authorization = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    roles.forEach((key) => {
      const val = key.permissions.filter(
        (x) => x.service.name.toLowerCase() == requestedService
      );
    ...
    if (service.length) {
      ...
      const readRestrictions = ["PUT", "DELETE"];

      if (accessType == "Read-Only" && readRestrictions.indexOf(method) != -1) {
        res.status(404).setCode(955);
        throw new Error(
          "You don't have permission to the access this API endpoint"
        );
      }
